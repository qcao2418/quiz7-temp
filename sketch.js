...
function preload() {
  img = loadImage("/assets/yugo-profile.png");
}
...
function draw() {
  ...
  if (drawSegments) {
    ...
    for (const segment of segments) {
      segment.draw();
      if (mouseX >= segment.srcImgSegXPos &&
        mouseX <= segment.srcImgSegXPos + segment.srcImgSegWidth &&
        mouseY >= segment.srcImgSegYPos &&
        mouseY <= segment.srcImgSegYPos + segment.srcImgSegHeight) {
        if (mouseIsPressed) {
          // If mouse is pressed, draw the original image
          segment.drawOriginal();
        }
        // else {
        //   stroke(255, 0, 0);
        // }
    ...
    for (const segment of segments) {
      if (mouseX >= segment.srcImgSegXPos && mouseX <= segment.srcImgSegXPos + segment.srcImgSegWidth && mouseY >= segment.srcImgSegYPos && mouseY <= segment.srcImgSegYPos + segment.srcImgSegHeight) {
        if (mouseIsPressed) {
          stroke(255, 255, 255);
        } else {
          stroke(255, 0, 0);
          strokeWeight(4);
          noFill();
          rect(segment.srcImgSegXPos, segment.srcImgSegYPos, segment.srcImgSegWidth, segment.srcImgSegHeight);
        }
